...
export default function RegistraciaPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    ico: '',
    dic: '',
    ic_dph: '', // Pridané IČ DPH
    address: '',
    city: '',
    zip: '',
    country: 'Slovensko', // Pridaná Krajina
    referral_source: '', // Odkiaľ sa dozvedeli
    gdpr: false
  });
  ...
  const handleRegister = async (e) => {
    ...
    if (authData.user) {
      ...
      // Email ukladáme aj sem pre ľahšie párovanie v Admine
      ...
      if (profileError) {
        alert("Chyba pri vytváraní profilu: " + profileError.message);
      } else {
        alert("Registrácia úspešná! Skontrolujte si e-mail (ak je vyžadované potvrdenie) a prihláste sa.");
        router.push('/login');
      }
    }
    setLoading(false);
    ...
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      <div className="bg-zinc-900 border border-zinc-800 p-8 md:p-12 rounded-[3rem] w-full max-w-3xl shadow-2xl animate-in fade-in zoom-in duration-500 my-10">
        ...
        <header className="text-center mb-10">
          <h1 className="text-4xl font-black uppercase italic tracking-tighter leading-none">Registrácia <span className="text-red-600">Zákazníka</span></h1>
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          <div className="flex bg-black p-1 rounded-2xl border border-zinc-800 mb-8">
            <button type="button" onClick={() => setClientType('Osoba')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${clientType === 'Osoba' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}>Súkromná Osoba</button>
            <button type="button" onClick={() => setClientType('Firma')} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase transition-all ${clientType === 'Firma' ? 'bg-red-600 text-white shadow-lg' : 'text-zinc-500 hover:text-white'}`}>Firma / Živnostník</button>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {/* ĽAVÝ STĹPEC: PRÍSTUP A OSOBNÉ ÚDAJE */}
            <div className="space-y-4">
              <h2 className="text-red-600 text-[10px] font-black uppercase tracking-widest ml-1">Prihlasovacie údaje</h2>
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <input required type="password" placeholder="Zopakovať heslo" value={formData.confirmPassword} onChange={(e) => setFormData({...formData, confirmPassword: e.target.value})} className="w-full bg-black border border-zinc-800 p-4 rounded-xl text-white outline-none focus:border-red-600 transition-all font-bold"/>
              </div>

              <h2 className="text-red-600 text-[10px] font-black uppercase tracking-widest ml-1 mt-6">Kontaktné údaje</h2>
              ...
              <select value={formData.referral_source} onChange={(e) => setFormData({...formData, referral_source: e.target.value})} className="w-full bg-black border border-zinc-800 p-4 rounded-xl text-zinc-400 outline-none focus:border-red-600 font-bold appearance-none">
                <option value="">Odkiaľ nás poznáte? (voliteľné)</option>
            ...
            {/* PRAVÝ STĹPEC: ADRESA A FIRMA */}
            <div className="space-y-4">
              {clientType === 'Firma' ? (
                <>
                  <h2 className="text-blue-500 text-[10px] font-black uppercase tracking-widest ml-1">Fakturačné údaje</h2>
                ...
                <div className="h-[50px] md:block hidden"></div> // Spacer pre zarovnanie
              )}
              
              <h2 className="text-red-600 text-[10px] font-black uppercase tracking-widest ml-1">Adresa trvalého bydliska / Sídlo</h2>
          ...
          <div className="flex items-start gap-3 bg-black/50 p-5 rounded-2xl border border-zinc-800 mt-4">
            ...
            <label htmlFor="gdpr" className="text-[9px] text-zinc-500 font-black uppercase tracking-widest leading-relaxed cursor-pointer">
          ...
          <button type="submit" disabled={loading} className="w-full bg-red-600 hover:bg-red-700 text-white font-black py-6 rounded-2xl uppercase text-xs tracking-[0.3em] shadow-xl shadow-red-900/20 transition-all disabled:opacity-50">
