  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (data) {
        ...
        if (found) {
          setItemData(typeof found === 'string' ? { title: found, desc: '', image_urls: [] } : found);
        }
      }
      setLoading(false);
    };
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans">
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 pb-24">
        ...
        {/* FOTKY — ak viac ako 1 a nie je bočný stĺpec (viac fotiek, celá šírka) */}
        {images.length > 2 && paragraphs.length > 0 && (
          <div className="mt-12">
            <div className="flex items-center gap-4 mb-6">
              <div className="h-[2px] w-8 bg-blue-500" />
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-400">Fotogaléria</p>
              <div className="h-[1px] flex-grow bg-gradient-to-r from-zinc-800 to-transparent" />
            </div>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {images.map((url, i) => (
                <div
                  key={i}
                  onClick={() => setLightbox(url)}
                  className={`rounded-2xl overflow-hidden border border-zinc-900 hover:border-blue-500/30 cursor-zoom-in transition-all group ${i === 0 ? 'col-span-2 md:col-span-2' : ''}`}
                >
                  <img src={url} alt="" className="w-full h-full object-cover aspect-video group-hover:scale-[1.02] transition-transform duration-500" />
                </div>
              ))}
            </div>
          </div>
