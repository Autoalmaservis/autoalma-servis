...
export default function DetailZakazkyPage() {
  ...
  const handleFinalizeJob = async (isOfficial) => {
    ...
    try {
      ...
      router.push('/faktury');
    } catch (err) { alert("Chyba: " + err.message); } finally { setInvoiceLoading(false); }
  };
  ...
  return (
    <div className="min-h-screen bg-black text-white p-4 md:p-12 relative font-sans font-bold">
      ...
      <div className="printable-area bg-zinc-900 border border-zinc-800 p-8 md:p-16 rounded-[3rem] shadow-2xl max-w-5xl mx-auto text-white">
        ...
        {/* KLIENT A CHECKLIST (Postrážené, aby nevypadol Checklist) */}
                  ...
                  <div key={task.id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-300 font-bold ${done ? 'bg-green-600/10 border-green-600 text-green-500 shadow-[0_0_15px_rgba(22,163,74,0.1)]' : 'bg-red-600/5 border-red-600/40 text-red-500 shadow-[0_0_10px_rgba(220,38,38,0.05)]'}`}>
                    <div className="flex items-center gap-4 cursor-pointer flex-grow no-print font-bold" onClick={() => toggleTaskStatus(task.id, task.is_completed)}>
                      <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${done ? 'bg-green-600 border-green-600 shadow-lg shadow-green-900/40 font-bold' : 'bg-transparent border-red-600 font-bold'}`}>
                        {done ? <span className="text-white text-xs font-black font-bold font-bold font-bold font-bold font-bold font-bold font-bold">✓</span> : <span className="text-red-600 text-[10px] font-black uppercase tracking-tighter font-bold">X</span>}
                      </div>
            ...
            <table className="w-full text-left text-sm font-bold font-bold">
              ...
              <tbody className="divide-y divide-zinc-800 font-black italic uppercase font-bold font-bold">
                ...
                <tr className="no-print bg-black/50 border-t-2 border-red-600/20 font-bold font-bold">
                  ...
                  <td className="p-3 font-bold font-bold font-bold font-bold font-bold font-bold"><button onClick={addItem} className="w-full bg-red-600 text-white font-black py-3 rounded-xl hover:bg-red-500 transition-all shadow-xl text-lg">+</button></td>
                  <td className="p-3 font-bold"></td>
                </tr>
              </tbody>
            </table>
            <div className="p-8 bg-zinc-800/20 flex flex-col items-end space-y-3 font-black italic uppercase font-bold font-bold font-bold font-bold">
              ...
              <div className="flex justify-between w-72 text-zinc-400 text-[11px] font-black uppercase tracking-widest pt-1 font-bold font-bold"><span>Základ dane:</span><span className="text-white font-bold font-bold">{subtotal.toFixed(2)} €</span></div>
              ...
              <div className="flex justify-between w-80 pt-4 font-bold font-bold font-bold font-bold font-bold font-bold font-bold font-bold"><span className="text-red-600 font-black uppercase italic tracking-tighter text-xl leading-none font-bold font-bold font-bold">Spolu k úhrade:</span><span className="text-4xl font-black italic tracking-tighter leading-none font-bold font-bold font-bold">{total.toFixed(2)} €</span></div>
            </div>
          </div>
        </div>
      ...
      {/* MODAL FAKTURÁCIA */}
      {isInvoiceModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[250] flex items-center justify-center p-6 no-print font-bold font-bold font-bold">
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-[200] flex items-center justify-center p-6 no-print font-bold font-bold font-bold">
          <div className="bg-zinc-900 border border-zinc-800 p-10 rounded-[3rem] max-sm w-full text-center shadow-2xl font-bold font-bold font-bold">
