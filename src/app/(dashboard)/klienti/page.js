...
export default function KlientiPage() {
  ...
  const fetchKlienti = async () => {
    ...
    vehiclesData?.forEach(v => {
      const majitel = Object.values(mapaKlientov).find(k => 
        (k.db_full_name && k.db_full_name === v.owner_name) || 
        (k.db_company_name && k.db_company_name === v.owner_name) ||
        (k.email && k.email === v.owner_email) ||
  ...
  const handleSaveCar = async (e) => {
    ...
    const payload = {
      ...
      license_plate: carForm.plate_number.toUpperCase().trim(),
    ...
    <div className="p-6 md:p-10 min-h-screen bg-black text-white select-none font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-1">
          <input type="text" placeholder="Hľadať partnera / ŠPZ..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-zinc-900 border border-zinc-800 p-5 rounded-2xl text-white font-bold outline-none focus:border-red-600 mb-6 shadow-inner font-bold" />
              ...
              <div key={i} className="relative group">
                <button onClick={() => nacitajVozidla(k)} className={`w-full text-left p-6 rounded-3xl border transition-all relative font-bold ${selectedKlient === k.customer_name ? 'bg-red-600 border-red-600 shadow-2xl' : 'bg-zinc-900/40 border-zinc-800 hover:border-zinc-700'}`}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-3">
                    {k.all_plates && k.all_plates.length > 0 ? k.all_plates.map(p => (
                ...
                <div className="absolute right-4 top-1/2 -translate-y-1/2 flex gap-2 font-bold">
                  <span onClick={(e) => { e.stopPropagation(); openEditClientModal(k); }} className="bg-black/50 p-3 rounded-xl text-xs hover:bg-white hover:text-black transition-all border border-zinc-800 cursor-pointer font-bold">✏️</span>
                  <span onClick={(e) => { e.stopPropagation(); handleDeleteKlient(k); }} className={`p-3 rounded-xl text-[10px] font-black uppercase transition-all border cursor-pointer font-bold ${confirmDeleteName === k.customer_name ? 'bg-white text-red-600 border-white font-bold' : 'bg-black/50 text-zinc-600 border-zinc-800 hover:text-red-500'}`}>{confirmDeleteName === k.customer_name ? 'ZMAZAŤ?' : '🗑️'}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {selectedKlient ? (
            <div className="space-y-10 animate-in fade-in duration-500 font-bold">
              <div className="flex justify-between items-center border-b border-zinc-900 pb-8">
                ...
                <button onClick={() => { setEditMode(false); setCarForm({id:'', plate_number: '', brand: '', model: '', vin_number: '', engine_volume: '', engine_power: '', year_produced: '', fuel_type: 'Diesel', mileage: '' }); setIsCarModalOpen(true); }} className="bg-zinc-800 hover:bg-zinc-700 text-white font-black px-6 py-3 rounded-2xl text-[10px] uppercase border border-zinc-700 shadow-lg font-bold">+ Pridať Vozidlo</button>
              </div>
              <div className="space-y-16">
                {vozidla.map((v) => (
                  <div key={v.id} className="bg-zinc-900/30 border border-zinc-800 p-8 rounded-[3.5rem] relative shadow-xl font-bold">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                       <div>
                          <div className="flex items-center gap-3 mb-6">
                            <span className="bg-white text-black px-5 py-2 rounded-xl font-black text-2xl tracking-widest shadow-2xl uppercase font-bold">{v.plate_number}</span>
                          ...
                          <h3 className="text-3xl font-black uppercase italic mb-4 font-bold">{v.car_brand_model}</h3>
                          <div className="space-y-2 bg-black/40 p-5 rounded-2xl border border-zinc-800 text-[11px] font-black uppercase tracking-widest text-zinc-400 italic font-bold">
                             <p className="flex justify-between border-b border-zinc-800 pb-2 font-bold"><span>VIN:</span> <span className="text-white font-mono font-bold">{v.vin_number || '---'}</span></p>
                             <p className="flex justify-between border-b border-zinc-800 py-2 font-bold"><span>Motor:</span> <span className="text-white font-bold">{v.engine_volume} ccm / {v.engine_power} kW</span></p>
                             <p className="flex justify-between pt-2 font-bold"><span>Km / Rok:</span> <span className="text-white font-bold">{v.mileage || 0} KM • {v.year_produced}</span></p>
                          </div>
                       </div>
                       <div className="space-y-4 font-bold">
                          <h4 className="text-[10px] font-black uppercase tracking-[0.3em] text-red-600 mb-4 flex items-center gap-2 italic font-bold"><span className="w-2 h-2 bg-red-600 rounded-full animate-pulse font-bold"></span> História návštev</h4>
                          <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar italic font-bold">
