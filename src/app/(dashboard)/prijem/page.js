...
function PrijemForm() {
  ...
  // 1. STAVY - ZOSÚLADENÉ S NOVOU SCHÉMOU DATABÁZY
  ...
  // 3. LOGIKA AUTODOPLNENIA - AKTUALIZOVANÉ NA NOVÉ STĹPCE
  useEffect(() => {
    const autoDoplnenie = async () => {
      ...
      if (spzToQuery && spzToQuery.length >= 4) {
        // Hľadáme v trvalej tabuľke vozidiel
        ...
        if (vData) {
          ...
          // Hľadáme profil partnera pre dotiahnutie kontaktov a fakturačných údajov
          ...
          setFormData(prev => ({
            ...
            vin_number: vData.vin_number || '', // OPRAVENÉ z 'vin'
            mileage: vData.mileage || '',
            engine_volume: vData.engine_volume || '', // OPRAVENÉ z 'engine_vol'
            engine_power: vData.engine_power || '',   // OPRAVENÉ z 'engine_pow'
            year_produced: vData.year_produced || '', // OPRAVENÉ z 'year'
  ...
  return (
    <div className="p-6 md:p-10 min-h-screen bg-black text-white font-sans font-bold">
      ...
      <form onSubmit={handleSave} className="max-w-[1200px] bg-zinc-900/40 border border-zinc-800 p-8 md:p-12 rounded-[3.5rem] shadow-2xl space-y-12">
        ...
        {/* SEKCIA: VOZIDLO (ZJEDNOTENÝ DIZAJN) */}
        ...
        {/* SEKCIA: KLIENT A FAKTURAČNÉ ÚDAJE */}
